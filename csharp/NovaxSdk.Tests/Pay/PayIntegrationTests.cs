...
public class PayIntegrationTests
{
    ...
    {
        var resp = await Client.Pay.TokenAsync(
            new PayTokenRequest("1234", 1m));
    ...
    {
        var resp = await Client.Pay.H5OrderAsync(
            new H5OrderRequest(await GetTokenAsync()));
        Assert.NotNull(resp);
    }

    [Fact] public async Task H5Protocols()
    {
        var resp = await Client.Pay.H5ProtocolsAsync(
            new H5ProtocolsRequest(await GetTokenAsync()));
        Assert.NotNull(resp);
    }
    ...
    {
        var resp = await Client.Pay.DynamicQrPayCreateAsync(
            new DynamicQrPayCreateRequest(
                ReceiptOrderId: Ts(),
                CompanyUserId:  88_888_896L,
                Currency:       "usdt",
                Amount:         1m,
                Comment:        "test"));
        // server returns order id in Msg
        Assert.NotNull(resp?.Msg);
    }
    ...
    {
        var resp = await Client.Pay.ReceiptProtocolsAsync(
            new ReceiptProtocolsRequest(ReceiptType: 1));
    ...
    {
        var resp = await Client.Pay.ReceiptProtocolsAsync(
            new ReceiptProtocolsRequest(ReceiptType: 6));
        Assert.NotNull(resp);
    }
    ...
    [Fact] public async Task ReceiptAddOrder()
    {
        var resp = await Client.Pay.ReceiptAddOrderAsync(new ReceiptAddOrderRequest(
            Protocol: "TRC20", Currency: "USDT",
            SmartContractAddress: "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t",
            CompanyUserId: "88888896", ReceiptOrderId: Ts(),
            CurrencyNumber: 1m,
